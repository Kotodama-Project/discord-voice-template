...
test('OpenAI whisper protocol sends file field, model and bearer key, and tolerates missing segments',async t=>{
  ...
  const fn=createWhisperArchiveAsr({sink:{verify:async()=>root},endpoint:'https://api.openai.com/v1/audio/transcriptions',authorize:()=>true,protocol:'openai',model:'whisper-1',apiKeyEnv:'FIXTURE_WHISPER_KEY',readEnv:()=>'fixture-key',
    fetcher:async(url,init)=>{seen.url=String(url);seen.auth=init.headers.authorization;const text=await new Response(init.body).text();seen.body=text;return new Response(JSON.stringify({text:'x',duration:1,language:'ja'}),{headers:{'content-type':'application/json'}});}});
  ...
  assert.deepEqual(segments,[]);assert.equal(seen.url,'https://api.openai.com/v1/audio/transcriptions');assert.equal(seen.auth,'Bearer fixture-key');
