...
export async function loadConfig(filename) {
  const config=Config.parse(await readJson(filename));check(Boolean(config.archive?.enabled)===config.voice.storeAudio,'ARCHIVE_RECORDING_CONFIG_REQUIRED');if(config.archive?.enabled){check(config.voice.transcriptSource==='local'&&config.voice.consentMode==='owner_managed'&&config.agentBinding&&config.owner.kind==='local','ARCHIVE_BINDING_REQUIRED');const endpoint=new URL(config.archive.whisperEndpoint);
    if(config.archive.whisperProtocol==='openai'){check(endpoint.protocol==='https:'&&endpoint.hostname==='api.openai.com'&&!endpoint.username&&!endpoint.password,'ARCHIVE_ASR_HOST_MISMATCH');check(config.archive.whisperApiKeyEnv,'ARCHIVE_ASR_CREDENTIAL_REQUIRED');}
    else check(['127.0.0.1','localhost',new URL(config.voice.localAsr.url).hostname].includes(endpoint.hostname),'ARCHIVE_ASR_HOST_MISMATCH');}
