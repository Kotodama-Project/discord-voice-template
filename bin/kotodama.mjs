...
try{
  ...
  if(command==='doctor'){
    ...
    output({node:process.versions.node,nodeSupported:Number(process.versions.node.split('.')[0])>=24,taskOwner:config.owner.kind,
      ...
      archiveConfigured:Boolean(config.archive?.enabled),whisperProtocol:config.archive?.whisperProtocol??null,localWriteWorkerSupported:process.platform!=='win32',providerVerified:false});
  ...
  }else if(['import-file','export','read-export'].includes(command)){
